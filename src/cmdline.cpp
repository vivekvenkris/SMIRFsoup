...
int read_cmdline_options(CmdLineOptions& args, int argc, char **argv)
{
	try
	{
		...
		TCLAP::ValueArg<std::string> arg_archives_base("A", "archives_base","The base directory to find archives", false, "", "string",cmd);

		TCLAP::ValueArg<std::string> arg_smirf_base("S", "smirf_base", "The base directory to smirf related files", false, "", "string",cmd);
		...
		if(args.host.empty()){
			...
			std::cerr<< "Instance BS_"<< args.beam_searcher_id << " on host:" << args.host <<std::endl;


		} else{

			std::cerr<< "Pretending to be BS_"<< args.beam_searcher_id << " on host:" << args.host <<std::endl;
		...
		if(!arg_dada_key.getValue().empty()) {
		...
		if(args.verbose){
			std::cerr << "Arguments to be used:"
					...
					<< arg_beam_searcher_name.getName() << "\t '" << arg_beam_searcher_name.getValue() << "' \n";
		}
...
int organize(CmdLineOptions& args){
	...
	if(args.smirf_base.empty()) args.smirf_base = ConfigManager::smirf_base();
	if(args.archives_base.empty()) args.archives_base = ConfigManager::archives_base();

	std::stringstream smirf_utc_dir_stream;
	smirf_utc_dir_stream << args.smirf_base << PATH_SEPERATOR << ConfigManager::bs_dir_prefix() << std::setfill('0') << std::setw(2) << args.beam_searcher_id << PATH_SEPERATOR << args.utc;
