...
public:
  ...
  */
  virtual ~ReusableDeviceTimeSeries()
  {
    Utils::device_free(copy_buffer);
  }
};
...
private:
  ...
  size_t max_delay; // added by vivek to know actual nsamp
...
//  }
  DispersionTrials(T* data_ptr, unsigned int nsamps, float tsamp, std::vector<float> dm_list_in, size_t max_delay)
     :TimeSeriesContainer<T>(data_ptr,nsamps,tsamp, (unsigned int)dm_list_in.size())
   {
     dm_list.swap(dm_list_in);
  ...
  */
  ...
  size_t get_max_delay(void){return max_delay;}
