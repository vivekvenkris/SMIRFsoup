...
public:
	...
	Zapper(std::map<float,float>* zap_map){

		d_mem_allocated = false;

		append_from_map(zap_map);

	}

	void append_from_map(std::map<float,float>* zap_map){

		for(std::map<float,float>::iterator it = zap_map->begin(); it != zap_map->end(); it++) {

			birdies.push_back(it->first);
